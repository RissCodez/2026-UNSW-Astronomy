...
tidyPlot = np.column_stack((arrayTidyRA, arrayTidyDEC))
...
plt.scatter(FR[:,0], FR[:,1], c='#f74f4f', label='FR', s=10)
plt.scatter(SR[:,0], SR[:,1], c='#4fcaf7', label='SR', s=10)
...
plt.axis('equal')
plt.xlabel('X (Radians)'); plt.ylabel('Y (Radians)')
plt.title(f'{clusterName}')
plt.legend(loc=legendLocation, fontsize=12)

plt.savefig(f'{clusterName}_kinematics_density.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.scatter(x=ellipticity[rotationList == True], y=lambdaList[rotationList == True],zorder=2, c='#f74f4f', label='FR', s=10)
plt.scatter(x=ellipticity[rotationList == False], y=lambdaList[rotationList == False],zorder=2, c='#4fcaf7', label='SR', s=10)
plt.ylabel('λR')
plt.xlabel('ε')
plt.xlim(0, 0.5)
...
plt.show()
F_SR = (len(SR)/(len(SR)+len(FR)))

print(f'{len(SR)+len(FR)}\t{len(SR)}\t{F_SR:.2%}')

import numpy as np          
from scipy.spatial import KDTree  
import matplotlib.pyplot as plt       

# create KDTree and grab 5 nearest neighbours (index 1-5) + self (index 0)
XY = np.column_stack([arrayTidyRA, arrayTidyDEC]) 
tree = KDTree(XY)                                
NN5, _ = tree.query(XY, k=6); NN5 = NN5[:, 4]  

densityBins = np.linspace(0, np.max(NN5), 10)  
iBins = np.digitize(NN5, densityBins) - 1 

fracSR = []                 
binGalaxyCount = []         
binCen = []                 

for i in range(len(densityBins)-1):
    bin_mask = (iBins == i)       
    sr_mask = bin_mask & (rotationList == False) 
    
    total_in_bin = np.sum(bin_mask)             
    sr_in_bin = np.sum(sr_mask)                 
    
    fraction = sr_in_bin / total_in_bin if total_in_bin > 0 else 0  
    fracSR.append(fraction)               
    binGalaxyCount.append(total_in_bin)           
    binCen.append(0.5*(densityBins[i] + densityBins[i+1]))  
...
df = pd.DataFrame({
    'Bin': range(len(binCen)),
    'Total': binGalaxyCount,
    'SR_Count': [int(binGalaxyCount[i] * fracSR[i]) for i in range(len(binCen))],
    'SR_Fraction': fracSR
})
df.to_csv(f'{clusterName}_density_bins.csv', index=False)
print(f"Saved to {clusterName}_density_bins.csv")


plt.figure(figsize=(8, 4))
plt.plot(binCen, fracSR, 'o-', alpha=0.8, color='blue')

for i in range(len(binCen)):
    
    # annotate FR's across whole plot
    FR = binGalaxyCount[i] - int(binGalaxyCount[i] * fracSR[i])
    plt.annotate(f'{FR}',                  
                xy=(binCen[i], fracSR[i]),       
                xytext=(binCen[i], 0.82),       
                textcoords='data',              
                ha='center', fontsize=9, fontweight='bold', color='red')
    
    # annotate SR's across whole plot
    plt.annotate(f'{int(binGalaxyCount[i]*fracSR[i])}', 
                xy=(binCen[i], fracSR[i]),         
                xytext=(binCen[i], 0.72),          
                textcoords='data',
                ha='center', fontsize=9, fontweight='bold', color='blue')
plt.xlabel(r'$\Sigma_5$')
plt.ylabel(r'$f_{SR}$')
plt.title(f'{clusterName}')
plt.grid(True, alpha=0.3)
plt.ylim(0, 1)
plt.savefig(f'{clusterName}_sr_fraction_density.jpg', dpi=300, bbox_inches='tight')
plt.show()                          
