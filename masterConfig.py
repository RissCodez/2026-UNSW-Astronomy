'''
clusterName is a string indicating which cluster is being worked on.

minSNR is an integer indicating the minimum signal-noise ratio to be kept,
any lower spaxels will be masked.

targetSNR is an integer indicating the target SNR for voronoi binning (if close
to minSNR there will be more bins, so closer to binning individual spaxels).
'''
...
def masterConfig(clusterName, minSNR, targetSNR):
    galaxyData = pd.read_csv(f"{clusterName}_masterConfigValues.csv")
    
    shFile = "source ngist_env/bin/activate\ncd ~/ngist_supplementary_public/gistTutorial"
    
    for i in range(len(galaxyData)):
        
        galaxy = galaxyData['MAGPIID'][i]
        ...
        masterConfig = f"""GENERAL :
  RUN_ID : 'MAGPI{galaxy}_SNR{MIN_SNR}' # Name of the analysis run. A subdirectory of this name within the output directory will be created. This identifier further serves as a prefix to all output files.
  INPUT : 'MAGPI{galaxy}_minicube.fits' # Input file for this analysis run. The specified path is relative to the input path given in defaultDir.
  ...
  REDSHIFT : {Z} # Initial guess on the redshift of the system [in z]. Spectra are shifted to rest-frame, according to this redshift.
...
READ_DATA :
  ...
  ORIGIN : {X},{Y} # Origin of the coordinate system in pixel coordinates: x,y (Indexing starts at 0). 
...
SPATIAL_MASKING :
  ...
  MIN_SNR : {minSNR} # Spaxels below the isophote level which has this mean signal-to-noise level are masked.
  MASK : 'MAGPI{galaxy}_mask.fits' # File containing a spatial mask (Set False to not include a file).
...
SPATIAL_BINNING :
  ...
  TARGET_SNR : {targetSNR} # Target signal-to-noise ratio for the Voronoi binning
...
KIN :
  ...
  LMAX : 7100
  SIGMA : 0 # Initial guess of the velocity dispersion of the system [in km/s]
  ...
  MOM : 2 # Number of kinematic moments to be extracted
...
CONT :
  METHOD : False # Name of the routine in stellarKinematics/ (without .py) to perform the tasks. Set False to turn off module. Set 'ppxf' to use the standard GIST implementation, exploiting the pPXF routine of Cappellari & Emsellem (2004).
  SPEC_MASK : 'specMask_KIN' # File to define wavelength ranges to be masked during the stellar kinematics fit. The specified path is relative to the configDir path in defaultDir.
  LMIN : 4750 # Rest-frame wavelength range used for the stellar kinematics analysis [in Angst.]
  LMAX : 7100
  SIGMA : 0 # Initial guess of the velocity dispersion of the system [in km/s]
...
GAS :
  METHOD : False # options are 'ppxf', 'gandalf' or 'MAGPI_gandalf' Name of the routine in emissionLines/ (without .py) to perform the tasks. Set False to turn off module. Set 'gandalf' to use the standard GIST implementation, exploiting the pyGandALF routine of Sarzi et al. (2006).
...
SFH :
  METHOD : False # Name of the routine in starFormationHistories/ (without .py) to perform the tasks. Set False to turn off module. Set 'ppxf' to use the standard GIST implementation, exploiting the pPXF routine of Cappellari & Emsellem (2004).
...
LS :
  METHOD : False # Name of the routine in lineStrengths/ (without .py) to perform the tasks. Set False to turn off module. Set 'default' to use the standard GIST implementation, exploiting the routines of Kuntschner et al. (2006) and Martin-Navarro et al. (2018).
...
UMOD :
  METHOD : False # Name of the routine in User Modoule / (without .py) to perform the tasks. Set False to turn off module. More info soon"""
        filePath = rf"masterConfig\MAGPI{galaxy}_masterConfig_SNR{minSNR}.yaml"
        fileName = rf"MAGPI{galaxy}_masterConfig_SNR{minSNR}.yaml"
        
        shellName = rf"masterConfig\run{clusterName}.sh"
        configPath = f"ngistPipeline --config configFiles/{fileName} --default-dir configFiles/defaultDir"
        
        shFile = shFile + f"\n{configPath}"
        
        with open(filePath, 'w') as yaml:
            yaml.write(masterConfig)
    
    with open(shellName, 'w') as sh:
        sh.write(shFile)
